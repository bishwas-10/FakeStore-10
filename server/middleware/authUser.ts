...
const refresh = (req: Request, res: Response, next: NextFunction) => {
  ...
  if (!cookie) {
    return res
      ...
      .send({
        status: false,
        message: "you are not authorized please logout and signin again",
      });
  }
    ...
    async (error: jwt.VerifyErrors | null, user: JwtPayload | undefined) => {
      if (error) {
        return res
          ...
          .send({
            status: false,
            message: "you are forbidden please logout and signin again",
          });
      }
  ...
  try {
    ...
    if (!token) {
      return res
        ...
        .send({
          status: false,
          message: "Token is missing please logout and signin again",
        });
    }
    ...
    jwt.verify(
      ...
      { complete: true },
      async function (error: VerifyErrors | null, decoded: any) {
        if (error) {
          if (error instanceof TokenExpiredError) {
            ...
            return res
              ...
              .send({
                status: false,
                message: "Token invalid please logout and signin again",
              });
          }
        }
        if (decoded) {
